...
export const SECTIONS: Section[] = [
  ...
  { id: 'workshop', title: '08 // МАСТЕРСКАЯ' },
  { id: 'outcomes', title: '09 // РЕЗУЛЬТАТЫ' },
  { id: 'faq', title: '10 // FAQ' }
];
...
export const GLOSSARY_CATEGORIES = [
  {
    ...
    terms: [
      ...
      { t: 'Fine-tuning (Дообучение)', d: 'Дообучение готовой модели на ваших данных, чтобы она лучше решала конкретную задачу.' },
      { t: 'Embedding (Эмбеддинг)', d: 'Числовое представление текста, позволяющее AI сравнивать смыслы слов и фраз.' },
      { t: 'Open Source (Открытый код)', d: 'Программы с открытым исходным кодом, которые можно использовать и изменять бесплатно (DeepSeek, Llama).' },
      { t: 'Бенчмарк', d: 'Стандартный тест для сравнения возможностей разных AI-моделей между собой.' },
      { t: 'Inference (Инференс)', d: 'Процесс, когда обученная модель генерирует ответ на ваш запрос. То, за что вы платите токенами.' }
    ]
  },
  {
    ...
    terms: [
      ...
      { t: 'Second Brain (Второй мозг)', d: 'Система организации знаний, где AI помогает собирать, структурировать и находить информацию.' },
      { t: 'Транскрибация', d: 'Автоматическое превращение речи в текст с помощью Whisper и других инструментов.' },
      { t: 'Демо-день', d: 'Финальная презентация проектов участников кэмпа в последний день программы.' }
    ]
  },
  {
    ...
    terms: [
      ...
      { t: 'IDE (Среда разработки)', d: 'Программа для написания кода с подсказками и инструментами (Cursor, VS Code).' },
      { t: 'CLI (Командная строка)', d: 'Интерфейс для управления программами через текстовые команды (например, Claude Code).' },
      { t: 'База данных (Database)', d: 'Организованное хранилище структурированных данных (PostgreSQL, Supabase).' },
      { t: 'Webhook (Вебхук)', d: 'Автоматическое уведомление от одного сервиса другому при наступлении события.' },
      { t: 'Commit (Коммит)', d: 'Точка сохранения в Git — фиксация изменений в коде с описанием, что было сделано.' },
      { t: 'Fork (Форк)', d: 'Копия чужого репозитория на GitHub, которую можно свободно изменять и развивать.' },
      { t: 'Лендинг (Landing Page)', d: 'Одностраничный сайт, созданный для презентации продукта или сбора заявок.' },
      { t: 'Бот (Bot)', d: 'Программа, автоматически выполняющая задачи в мессенджере (Telegram) или другом сервисе.' }
    ]
  },
  {
    ...
    terms: [
      ...
      { t: 'TypeScript', d: 'Расширение JavaScript с проверкой типов данных — помогает AI писать более надежный код.' },
      { t: 'Next.js', d: 'Популярный фреймворк для создания сайтов, который используется в Cursor и v0.' },
      { t: 'SQL', d: 'Язык запросов к базам данных — позволяет искать, добавлять и изменять данные.' }
    ]
  },
  {
    ...
    terms: [
      ...
      { t: 'Прототип', d: 'Быстрая рабочая версия продукта для проверки идеи, созданная за часы, а не месяцы.' },
      { t: 'Дебаг (Debug)', d: 'Процесс поиска и исправления ошибок в коде.' },
      { t: 'Рефакторинг', d: 'Улучшение структуры существующего кода без изменения его функциональности.' },
      { t: 'Дизайн-токены', d: 'Набор значений (цвета, шрифты, отступы), задающих единый визуальный стиль проекта.' }
    ]
  },
  {
    ...
    terms: [
      ...
      { t: 'CI/CD', d: 'Автоматическая сборка и публикация проекта при каждом изменении кода.' },
      { t: 'SSL / HTTPS', d: 'Шифрование соединения между браузером и сайтом (замочек в адресной строке).' },
      { t: 'npm (Пакетный менеджер)', d: 'Инструмент для установки готовых библиотек и зависимостей в проект.' },
      { t: 'Serverless (Бессерверный)', d: 'Подход, где сервер запускается только при обращении — не нужно управлять инфраструктурой (Vercel, Netlify).' },
      { t: 'Миграция', d: 'Изменение структуры базы данных: добавление таблиц, колонок или связей.' }
    ]
  },
  {
    ...
    terms: [
      ...
      { t: 'Парсинг', d: 'Автоматическое извлечение нужных данных из веб-страниц, документов или файлов.' },
      { t: 'Суммаризация', d: 'Сжатие длинного текста в краткую выжимку с сохранением ключевых идей.' },
      { t: 'Промпт-шаблон (Prompt Template)', d: 'Готовая структура запроса с переменными, которую можно переиспользовать для разных задач.' }
    ]
  },
  {
    ...
    terms: [
      ...
      { t: 'Воркспейс (Workspace)', d: 'Рабочее пространство в IDE или сервисе, объединяющее файлы и настройки проекта.' },
      { t: 'Boilerplate (Бойлерплейт)', d: 'Шаблон проекта с базовой настройкой, от которого можно начать разработку.' },
      { t: 'README', d: 'Файл-инструкция в корне проекта, описывающая что это за проект и как его запустить.' },
      { t: 'Автоматизация', d: 'Настройка процессов, которые выполняются без вашего участия (боты, скрипты, CI/CD).' }
    ]
  }
];
...
export const PROJECTS: ProjectExample[] = [
  ...
  { category: 'Telegram-боты', title: 'Дедлайн-бот', why: 'Командный контроль сроков и задач.', features: ['Анализ календаря', 'Уведомления о дедлайнах', 'Статус задач'] },
  { category: 'Telegram-боты', title: 'Конвертер валют', why: 'Быстрый пересчёт курсов прямо в чате.', features: ['Актуальные курсы', 'Мульти-валюта', 'История конвертаций'] },
  { category: 'Telegram-боты', title: 'Гид-путеводитель', why: 'Персональный помощник для путешествий по городу или региону.', features: ['Поиск локаций', 'Построение маршрутов', 'Рекомендации'] },
  { category: 'Telegram-боты', title: 'Бот-трекер здоровья питомца', why: 'Забота о здоровье домашнего животного.', features: ['Голосовой ввод симптомов', 'Напоминания о лекарствах', 'Дашборд здоровья'] },
  { category: 'Telegram-боты', title: 'Планировщик публикаций', why: 'CRM для ведения блога и соцсетей.', features: ['Контент-календарь', 'Статус черновиков', 'Аналитика постов'] },
  ...
  { category: 'Сайты и лендинги', title: 'Интерактивный дашборд из документа', why: 'Превращение скучного отчёта в живую визуализацию.', features: ['Импорт Word/PDF', 'Графики и диаграммы', 'Фильтры данных'] },
  { category: 'Сайты и лендинги', title: 'Мини-приложение для настолки', why: 'Цифровой помощник для любимой настольной игры.', features: ['Кнопка «Следующий ход»', 'Подсчёт очков', 'Таймер'] },
  { category: 'Сайты и лендинги', title: 'Веб-калькулятор / утилита', why: 'Решение конкретной бытовой задачи через простое приложение.', features: ['Формулы и расчёты', 'Сохранение результатов', 'Мобильная версия'] },
  ...
  { category: 'Системы и инструменты', title: 'Парсер и конвертер данных', why: 'Автоматическая обработка документов из одного формата в другой.', features: ['HTML → Markdown', 'PDF → текст', 'Массовая обработка'] },
  { category: 'Системы и инструменты', title: 'Мульти-бот платформа', why: 'Единая инфраструктура для управления несколькими ботами.', features: ['GitHub + Railway + БД', 'Общие настройки', 'Мониторинг'] },
  { category: 'Системы и инструменты', title: 'Скрипт-парсер сайтов', why: 'Автоматический сбор данных с любого сайта.', features: ['Извлечение таблиц', 'Фильтрация', 'Экспорт в CSV/JSON'] },
  { category: 'Системы и инструменты', title: 'AI-прототипы для бизнеса', why: 'Быстрая автоматизация бизнес-процессов без программистов.', features: ['Микросервисы', 'Интеграция с API', 'Замена ручной работы'] },
  { category: 'Системы и инструменты', title: 'Агент-составитель договоров и смет', why: 'Автоматизация рутинной юридической и финансовой документации.', features: ['Генерация договоров по шаблону', 'Расчёт смет', 'Экспорт в .docx/.xlsx'] },
  { category: 'Системы и инструменты', title: 'Агент-презентатор', why: 'Создание презентаций в заданном фирменном стиле по текстовому запросу.', features: ['Дизайн-токены бренда', 'Генерация слайдов', 'Экспорт в PDF/PPTX'] },
  ...
  { category: 'Визуальный контент', title: 'Визуализатор данных', why: 'Превращение любых данных в красивые графики и анимации.', features: ['Импорт CSV/JSON', 'Интерактивные графики', 'Экспорт изображений'] },
  { category: 'Визуальный контент', title: 'Голосовая мини-игра', why: 'Развлекательное приложение с управлением голосом.', features: ['Распознавание речи', 'Игровая логика', 'Озвучка ответов'] },
...
export const FAQ: FAQItem[] = [
  ...
  { category: 'Общие вопросы', question: 'Что такое AI Camp?', answer: 'За 4 недели участники учатся создавать боты, сайты и системы, используя AI как соавтора, без навыков программирования, и развивают AI-мышление для решения любых задач.' },
  ...
  { category: 'Общие вопросы', question: 'Какой формат обучения?', answer: 'Онлайн. Смешанный формат: 6 эфиров (1.5–2 часа каждый), 10+ уроков в записи, гайды и документы. Выходные дни включены для отдыха и практики.' },
  { category: 'Общие вопросы', question: 'Сколько длится кэмп?', answer: '4 недели.' },
...
export const WORKSHOP_TOOLS: WorkshopTool[] = [
  {
    id: 'claude-code',
    name: 'Claude Code',
    description: 'Терминальный AI-ассистент от Anthropic для разработки, рефакторинга и автоматизации.',
    resources: [
      {
        title: 'CLAUDE.md — Best Practices',
        description: 'Файл-инструкция от Бориса Черни, создателя Claude Code. Собирает лучшие практики из внутренних приёмов команды Anthropic. Накопительный эффект: каждую правку фиксируешь как правило, и со временем Claude меньше косячит.',
        type: 'claude-md',
...
- Minimal Impact: Changes should only touch what's necessary. Avoid introducing bugs.`
      ...
      {
        title: 'Библиотека промптов Anthropic',
        description: 'Официальная библиотека промптов от создателей Claude — готовые шаблоны для разных задач.',
        type: 'link',
